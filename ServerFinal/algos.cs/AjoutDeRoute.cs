...
namespace GenerationPlateau
{
    public class AjoutDeRoute
    {
        ...
        public int SupprimerCarteWagon(int couleur, int nombre)
        {
            ...
            {
                if (nombre != 0 && cartesEnMain[i].Identifiant == couleur)
                {
        ...
        {
            List<CarteWagon> cartesEnMain = Player.Wagons;
            List<int> nbCartes = [0, 0];
            for (int i = 0; i < cartesEnMain.Count; i++)
            {
                if (cartesEnMain[i].Identifiant == 0)
                {
                    nbCartes[1]++;
                }
                else if (cartesEnMain[i].Identifiant == couleur)
                {
                    nbCartes[0]++;
                }
            }
        ...
        public bool AjouterRoute()
        {
            ...
            int couleur = a.GetCouleur();
            int longueur = a.GetLongueur();
            int suppression;
