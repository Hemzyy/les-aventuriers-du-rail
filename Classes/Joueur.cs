...
public class Joueur
{
	...
	public Joueur(string Nom, int Identifiant, int Couleur, GestionnairePioche pileManager)
	{
		this.Nom = Nom;
	...
	public void piocherMission(int[] indexCartes)
	{
		var temporaryDraw = pileManager.piocherCarteMission();
		if (indexCartes == null)
		{
			indexCartes = new int[1];
			indexCartes[0] = temporaryDraw.Length - 1;
		}
		for (int i = 0; i < temporaryDraw.Length; i++)
		{
			if (indexCartes.Contains(i))
			{
				ajouterMission(temporaryDraw[i]);
			}
			else
			{
				pileManager.defausserMission(temporaryDraw[i]);
			}
	...
	public void piocherPileWagon()
	{
		ajouterWagon(pileManager.piocherCartesWagon());
	}

	// fonction qui permet de piocher a partir des cartes disponibles sur table 
	public void piocherTableWagon(int indexCarte)
	{
		ajouterWagon(pileManager.piocherSurTable(indexCarte));
	}
